...
async function checkPlayerDirect(playerName, teamSearch) {
  try {
    ...
    if (match) {
        ...
        const pRes = await fetch(`https://v3.football.api-sports.io/players/seasons?player=${match.id}`, { headers });
        const pData = await pRes.json();
        console.log("Seasons available for Kevin Arevalo:", pData.response);
        
        if (pData.response && pData.response.length > 0) {
            const latestSeason = pData.response[pData.response.length - 1];
            console.log(`Fetching stats for season ${latestSeason}...`);
            const pRes2 = await fetch(`https://v3.football.api-sports.io/players?id=${match.id}&season=${latestSeason}`, { headers });
